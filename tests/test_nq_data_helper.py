...
class TestNqData:
    ...
    async def _test_read_data_file(self, prompting_mode: str, num_examples: Optional[int] = 3) -> None:
        raw_data: List[SingleQuestionRawData] = await read_data_file(prompting_mode=PromptingMode(prompting_mode), 
    ...
    async def _test_get_golden_idx_change_data(
        ...
        log_steps: Optional[int] = 2
    ) -> None:
    
        res_data = await get_golden_idx_change_data(prompting_mode=PromptingMode(prompting_mode),
                                                    num_examples=num_examples,
                                                    model_name=model_name,
                                                    num_idxs=num_idxs,
                                                    path=path,
                                                    log_steps=log_steps)
        if isinstance(res_data, AsyncIterator):
            aggregated_results = []
            async for idx_data in res_data:
                aggregated_results.append(idx_data)
        else:
            aggregated_results = res_data

        src_data_payload: Dict = await self._read_results_data(file_name=f"get_golden_idx_change_data_{prompting_mode}.json")
        src_data = [SingleIdxData(**payload) for payload in src_data_payload]
        assert_that(aggregated_results).is_equal_to(src_data)
